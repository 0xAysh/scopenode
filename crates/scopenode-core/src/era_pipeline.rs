...
/// Convert a core [`StoredEvent`] to the storage row type.
///
/// SQLite stores hashes as hex strings and block numbers as i64.
pub fn core_to_storage_event(
    e: &crate::types::StoredEvent,
    ...
    scopenode_storage::models::StoredEvent {
        ...
        raw_topics: serde_json::to_string(
            &e.raw_topics.iter().map(|t| t.to_string()).collect::<Vec<_>>(),
        )
        .unwrap_or_default(),
